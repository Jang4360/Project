...
export class PaymentsService {
    ...
    async findAll() {
        ...
        try {
            ...
            console.log(
                `당신이 가진 돈에서 세금을 제외하면 ${result}원 입니다.`,
            ); // >>>>>>>>>>> 2. 세금 제외한 금액 기록
            ...
            setTimeout(async () => {
                const paymentCommission = await queryRunner.manager.findOne(
                    Payment,
                    {
                        where: { id: "d2ee2dea-816e-4aa0-9f19-e10b94ffe464" },
                ...
                const result =
                    paymentCommission.amount - paymentCommission.commission;
                console.log(
                    `당신이 가진 돈에서 수수료를 빼면 ${result}원 입니다.`,
                ); // >>>>>>>>>>> 3. 수수료 제외한 금액 기록
            }, 5000);
        } catch (error) {
